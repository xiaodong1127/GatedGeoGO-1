...
class RobertaLMHead(nn.Module):
    """Head for masked language modeling.这个类的目的是用于掩码语言建模（Masked Language Modeling），即在某些词被隐藏的情况下，预测这些词。"""
...
class ContactPredictionHead(nn.Module):
    ...
    该模块的主要功能是在给定的特征（通常是注意力机制的输出）上执行对称化操作和平均乘积校正（APC），然后计算逻辑回归以预测蛋白质之间或蛋白质内部的接触图。"""
...
class NormalizedResidualBlock(nn.Module):
    """这里的类是对TransformerLayer和AxialTransformerLayer的封装，主要是为了实现残差连接和LayerNorm。"""
    def __init__(
            self,
            layer: nn.Module,
            embedding_dim: int,
